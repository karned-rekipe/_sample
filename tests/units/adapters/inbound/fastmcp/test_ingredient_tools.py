...
async def client(service, logger, mcp_app):
    with patch("arclith_sample.adapters.inbound.fastmcp.tools.ingredient_tools.inject_tenant_uri", new=AsyncMock()):
        with patch("arclith_sample.adapters.inbound.fastmcp.tools.ingredient_tools.require_auth_mcp", new=AsyncMock(return_value={"sub": "test-user"})):
            with patch("arclith_sample.adapters.inbound.fastmcp.tools.admin_tools.require_auth_mcp", new=AsyncMock(return_value={"sub": "test-user"})):
